...
Tensor Tensor::To(Device device) {
    ...
    case DeviceType::kCUDA:
        ...
        break;
#else
    // 如果没有开启 USE_CUDA 编译，遇到 CUDA 设备直接降级回 CPU，避免直接崩溃
    case DeviceType::kCUDA:
    case DeviceType::kCPU:
        new_tensor = Tensor(*this, offset_, dims_);
        break;
#endif
    default:
...
std::shared_ptr<Tensor> Tensor::Flatten(int64_t start, int64_t end) {
    ...
    // 有faltten(1, -1)这样的写法，所以需要都变成顺序的写法
...
void Tensor::Backward(std::shared_ptr<Tensor> gradient, bool retain_graph, bool create_graph) const {
    ...
    // 需要解除const限制
    auto &mutable_grad = const_cast<std::shared_ptr<Tensor> &>(grad_);
    //累积tensor 多输出场景
    if (!mutable_grad) {
        mutable_grad = gradient;
    } else {
        mutable_grad = mutable_grad->Add(gradient);
    }

    // 利用function里面的backwardPartial,调用grad_fn_自己的这个，function内部自动触发
    // 自动反向传播；同时自己就有判空保护NULL的时候
    if (grad_fn_) {
