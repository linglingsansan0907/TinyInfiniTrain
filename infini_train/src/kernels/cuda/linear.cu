...
std::shared_ptr<Tensor> MatmulForward(const std::shared_ptr<Tensor> &input, const std::shared_ptr<Tensor> &other) {
    ...
    *output_dims.rbegin() = N;
    ...
    // 配置cuBLAS，只识别列优先，而C++是存储行优先 所以算CT = BT * AT即可
    float alpha = 1.0f;
    float beta = 0.0f;
    ...
    for (int64_t b = 0; b < bs; ++b) {
        const float *A_ptr = static_cast<const float *>(other->DataPtr()) + b * K * N;
        const float *B_ptr = static_cast<const float *>(input->DataPtr()) + b * M * K;
        float *C_ptr = static_cast<float *>(output->DataPtr()) + b * M * N;
...
MatmulBackward(const std::shared_ptr<Tensor> &input, const std::shared_ptr<Tensor> &other,
    ...
    const int64_t total_elements = std::accumulate(input_dims.begin(), input_dims.end(), 1LL, std::multiplies<int64_t>{});
    const int64_t bs = total_elements / (M * K);
    //把grad_input（M，K）和grad_other （K， N）创建在CUDA Tensor
    ...
    //每个batch求两个梯度矩阵
    for (int64_t b = 0; b < bs; ++b) {
        const float *dY = static_cast<const float *>(grad_output->DataPtr()) + b * M * N;
        const float *X = static_cast<const float *>(input->DataPtr()) + b * M * K;
        const float *W = static_cast<const float *>(other->DataPtr()) + b * K * N;

        float *dX = static_cast<float *>(grad_input->DataPtr()) + b * M * K;
        float *dW = static_cast<float *>(grad_other->DataPtr()) + b * K * N;
